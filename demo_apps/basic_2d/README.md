...
This demo is a compact 2D reference application included mainly for
completeness. It reads a model problem from JSON, builds the corresponding
geometry and PDE data, and evaluates the solution on a regular output grid.
The demo supports Laplace, Poisson, and screened Poisson equations with
...
`wos`, `wost`, `bvc`, or `rws`. Walk on Stars and Boundary Value Caching are
available in the CPU and GPU demos, in both C++ and Python. Reverse Walk on
Stars is CPU-only. The demo also includes a Walk on Spheres path for simpler
absorbing-boundary problems.
...
Run CPU solver configurations from the build directory as follows:

```bash
cd build
./demo_apps/basic_2d_demo ../demo_apps/basic_2d/model_problems/engine/wost.json
./demo_apps/basic_2d_demo ../demo_apps/basic_2d/model_problems/engine/bvc.json
./demo_apps/basic_2d_demo ../demo_apps/basic_2d/model_problems/engine/rws.json
```

Run the GPU-supported configurations with:

```bash
cd build
./demo_apps/basic_2d_gpu_demo ../demo_apps/basic_2d/model_problems/engine/wost.json
./demo_apps/basic_2d_gpu_demo ../demo_apps/basic_2d/model_problems/engine/bvc.json
```

The engine results are written to
`demo_apps/basic_2d/model_problems/engine/solutions`.

## Running the Python Demo

Run the CPU Python demo from the repository root:

```bash
python demo_apps/basic_2d/demo.py --config=demo_apps/basic_2d/model_problems/engine/wost.json
python demo_apps/basic_2d/demo.py --config=demo_apps/basic_2d/model_problems/engine/bvc.json
python demo_apps/basic_2d/demo.py --config=demo_apps/basic_2d/model_problems/engine/rws.json
```

Run the GPU Python demo with:

```bash
python demo_apps/basic_2d/gpu_demo.py --config=demo_apps/basic_2d/model_problems/engine/wost.json
python demo_apps/basic_2d/gpu_demo.py --config=demo_apps/basic_2d/model_problems/engine/bvc.json
```

## Custom Model Problem Creation

The WoSX 2D demo allows custom model problems to be created by specifying a boundary geometry, a reflecting (Neumann or Robin) boundary mask, and boundary and source textures. The reflecting boundary indicator will determine whether a boundary is Dirichlet (black) or Neumann/Robin (white). The mapping from scene space to the mask is computed relative to the bounding box for the boundary geometry.
...
```
{
    ...
    "modelProblem": {
        "geometry": "demo_apps/basic_2d/model_problems/engine/data/geometry.obj",
        "isReflectingBoundary": "demo_apps/basic_2d/model_problems/engine/data/is_reflecting_boundary.pfm",
        "absorbingBoundaryValue": "demo_apps/basic_2d/model_problems/engine/data/absorbing_boundary_value.pfm",
        "reflectingBoundaryValue": "demo_apps/basic_2d/model_problems/engine/data/reflecting_boundary_value.pfm",
        "sourceValue": "demo_apps/basic_2d/model_problems/engine/data/source_value.pfm",
        "robinCoeff": 0.0,
        "absorptionCoeff": 0.0,
    ...
    "output": {
        "solutionFile": "demo_apps/basic_2d/model_problems/engine/solutions/wost.pfm",
        "gridRes": 256,
