...
The full WoSX API, including treatment of 3D domains and/or vector-valued PDEs, can be viewed
...
def setup_pde(model_problem_config, bounding_box, dim, channels):
    ...
    # setup the PDE
    pde = wosx.Core.PDE(dim=dim, channels=channels)
    pde.source = wosx.Utils.get_dense_grid_source_callback(
        source_value_buffer, source_value_shape,
    ...
    if solve_double_sided:
        ...
        pde.dirichlet = wosx.Utils.get_dense_grid_dirichlet_callback(
        ...
        pde.robin = wosx.Utils.get_dense_grid_robin_callback(
        ...
        pde.robin = wosx.Utils.get_dense_grid_robin_callback(
            reflecting_boundary_value_buffer, reflecting_boundary_value_shape,
            domain_min, domain_max, dim=dim, channels=channels)
    pde.has_reflecting_boundary_conditions = wosx.Utils.get_dense_grid_indicator_callback(
        is_reflecting_boundary_buffer, is_reflecting_boundary_shape,
        domain_min, domain_max, dim=dim)
    pde.robin_coeff = wosx.Core.get_constant_robin_coefficient_callback(robin_coeff, dim=dim)
...
def setup_geometric_queries(model_problem_config, bounding_box,
    ...
    domain_min = bounding_box[0]
    domain_max = bounding_box[1]
    geometric_queries = wosx.Core.GeometricQueries(domain_is_watertight, domain_min, domain_max, dim=dim)

    # use an absorbing boundary handler to populate geometric queries for the absorbing boundary
    dirichlet_boundary_handler = wosx.Utils.FcpwDirichletBoundaryHandler(dim=dim)
    dirichlet_boundary_handler.build_acceleration_structure(absorbing_boundary_positions,
                                                            absorbing_boundary_indices)
    wosx.Utils.populate_geometric_queries_for_dirichlet_boundary(dirichlet_boundary_handler,
    ...
    if not solve_double_sided and use_sdf_for_absorbing_boundary:
        # override distance queries to use an SDF grid. The user can also use WoSX to build
        # an SDF hierarchy for double-sided problems (ommited here for simplicity)
        sdf_grid_for_dirichlet_boundary = wosx.Utils.SDFGrid(domain_min, domain_max, dim=dim)
        sdf_grid_shape = np.array([sdf_grid_resolution] * dim, dtype=np.int32)
        wosx.Utils.populate_sdf_grid(dirichlet_boundary_handler,
                                     sdf_grid_for_dirichlet_boundary,
                                     sdf_grid_shape, dim=dim)
        wosx.Utils.populate_geometric_queries_for_dirichlet_boundary(sdf_grid_for_dirichlet_boundary,
                                                                     geometric_queries, dim=dim)
    ...
    branch_traversal_weight = wosx.Utils.get_branch_traversal_weight_callback()

    if are_robin_conditions_pure_neumann:
        neumann_boundary_handler = wosx.Utils.FcpwNeumannBoundaryHandler(dim=dim)
        ...
        wosx.Utils.populate_geometric_queries_for_neumann_boundary(neumann_boundary_handler,
        ...
        robin_boundary_handler = wosx.Utils.FcpwRobinBoundaryHandler(dim=dim)
        ...
        wosx.Utils.populate_geometric_queries_for_robin_boundary(robin_boundary_handler,
                                                                 branch_traversal_weight,
                                                                 geometric_queries, dim=dim)
...
def compute_distance_info(solve_locations, geometric_queries,
    ...
    wosx.Utils.compute_dist_to_boundary(geometric_queries, solve_locations,
...
def invert_exterior_problem(kelvin_transform, positions, absorbing_boundary_positions,
    ...
    pde_inverted_domain = wosx.Core.PDE(dim=dim, channels=channels)
...
def create_sample_points(solve_locations, distance_info, dim, channels):
    ...
    for i in range(len(solve_locations)):
        pt = solve_locations[i]
        normal = np.zeros(dim)
        sample_type = wosx.Solvers.SampleType.InDomain
        in_valid_solve_region = distance_info[i][0]
        estimation_quantity = wosx.Solvers.EstimationQuantity.Solution\
                                if in_valid_solve_region else wosx.Solvers.EstimationQuantity.Skip
        pdf = 1.0
        dist_to_absorbing_boundary = distance_info[i][1]
        dist_to_reflecting_boundary = distance_info[i][2]

        sample_points[i] = wosx.Solvers.SamplePoint(pt, normal, sample_type,
                                                    estimation_quantity, pdf,
                                                    dist_to_absorbing_boundary,
                                                    dist_to_reflecting_boundary,
                                                    dim=dim, channels=channels)

    return wosx.Solvers.SamplePointList(sample_points, dim=dim, channels=channels)
...
def run_walk_on_spheres(solver_config, sample_pts, geometric_queries, pde,
    ...
    progress_bar = wosx.Utils.ProgressBar(n_samples)
    report_progress = wosx.Utils.get_report_progress_callback(progress_bar)

    walk_settings = wosx.Solvers.WalkSettings(epsilon_shell_for_absorbing_boundary,
    ...
    n_walks_list = wosx.IntList([n_walks] * n_samples)
    sample_statistics = wosx.Solvers.create_sample_statistics_list(n_samples, dim=dim, channels=channels)
    walk_on_spheres = wosx.Solvers.WalkOnSpheres(geometric_queries, dim=dim, channels=channels)
...
def run_walk_on_stars(solver_config, sample_pts, geometric_queries, pde,
    ...
    n_walks_list = wosx.IntList([n_walks] * n_samples)
    sample_statistics = wosx.Solvers.create_sample_statistics_list(n_samples, dim=dim, channels=channels)
    walk_on_stars = wosx.Solvers.WalkOnStars(geometric_queries, dim=dim, channels=channels)
...
def create_domain_sampler(geometric_queries, solve_double_sided, dim, channels):
    ...
    if solve_double_sided:
        ...
        return wosx.Samplers.create_uniform_domain_sampler(
...
def create_boundary_sampler(positions, indices, geometric_queries,
    ...
    if dim == 2:
        boundary_sampler = wosx.Samplers.create_uniform_line_segment_boundary_sampler(
                            positions, indices, geometric_queries.inside_bounding_domain,
                            dim=dim, channels=channels)
    else:
        boundary_sampler = wosx.Samplers.create_uniform_triangle_boundary_sampler(
                            positions, indices, geometric_queries.inside_bounding_domain,
                            dim=dim, channels=channels)
...
def create_bvc_evaluation_points(solve_locations, distance_info, dim, channels):
    ...
    for i in range(len(solve_locations)):
        ...
        evaluation_points[i] = wosx.Solvers.BVCEvaluationPoint(pt, normal, sample_type,
                                                               dist_to_absorbing_boundary,
                                                               dist_to_reflecting_boundary,
                                                               dim=dim, channels=channels)

    return wosx.Solvers.BVCEvaluationPointList(evaluation_points, dim=dim, channels=channels)
...
def run_boundary_value_caching(solver_config, evaluation_pts,
    ...
    progress_bar = wosx.Utils.ProgressBar(total_work)
    report_progress = wosx.Utils.get_report_progress_callback(progress_bar)

    boundary_value_caching = wosx.Solvers.BoundaryValueCaching(
...
def create_rws_evaluation_points(solve_locations, distance_info, dim, channels):
    ...
    for i in range(len(solve_locations)):
        ...
        evaluation_points[i] = wosx.Solvers.RWSEvaluationPoint(pt, normal, sample_type,
                                                               dist_to_absorbing_boundary,
                                                               dist_to_reflecting_boundary,
                                                               dim=dim, channels=channels)

    return wosx.Solvers.RWSEvaluationPointList(evaluation_points, dim=dim, channels=channels)
...
def run_reverse_walk_on_stars(solver_config, evaluation_pts,
    ...
    reverse_walk_on_stars = wosx.Solvers.ReverseWalkOnStars(geometric_queries,
...
def run_demo():
    # parse arguments
    parser = argparse.ArgumentParser(description="wosx 2d demo application")
