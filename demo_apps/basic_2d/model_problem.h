...
// in WoSX is templated on the dimension and value type of the PDE.
...
public:
    ...
    const wosx::PDE<T, 2>& getPDE() { return mPde; }
    const wosx::GeometricQueries<2>& getGeometricQueries() { return mQueries; }
    ...
    const wosx::PDE<T, 2>& getPDEInvertedDomain() { return mPdeInvertedDomain; }
    const wosx::GeometricQueries<2>& getGeometricQueriesInvertedDomain() { return mQueriesInvertedDomain; }
...
protected:
    ...
    void setupGeometricQueries(const std::vector<Vector2>& absorbingBoundaryPositions,
                               ...
                               std::unique_ptr<wosx::SdfGrid<2>>& sdfGridForAbsorbingBoundary,
                               wosx::FcpwDirichletBoundaryHandler<2>& absorbingBoundaryHandler,
                               wosx::FcpwNeumannBoundaryHandler<2>& reflectingNeumannBoundaryHandler,
                               wosx::FcpwRobinBoundaryHandler<2>& reflectingRobinBoundaryHandler,
                               wosx::GeometricQueries<2>& queries);
    ...
    std::unique_ptr<wosx::SdfGrid<2>> mSdfGridForAbsorbingBoundary;
    wosx::FcpwDirichletBoundaryHandler<2> mAbsorbingBoundaryHandler;
    wosx::FcpwNeumannBoundaryHandler<2> mReflectingNeumannBoundaryHandler;
    wosx::FcpwRobinBoundaryHandler<2> mReflectingRobinBoundaryHandler;
    wosx::GeometricQueries<2> mQueries;
    ...
    wosx::PDE<T, 2> mPdeInvertedDomain;
    std::vector<float> mMinRobinCoeffValuesInvertedDomain;
    std::vector<float> mMaxRobinCoeffValuesInvertedDomain;
    std::unique_ptr<wosx::SdfGrid<2>> mSdfGridForInvertedAbsorbingBoundary;
    wosx::FcpwDirichletBoundaryHandler<2> mInvertedAbsorbingBoundaryHandler;
    wosx::FcpwNeumannBoundaryHandler<2> mInvertedReflectingNeumannBoundaryHandler;
    wosx::FcpwRobinBoundaryHandler<2> mInvertedReflectingRobinBoundaryHandler;
    wosx::GeometricQueries<2> mQueriesInvertedDomain;
};
...
void ModelProblem<T>::partitionBoundaryMesh()
{
    ...
    wosx::partitionBoundaryMesh<2>(mPde.hasReflectingBoundaryConditions, mPositions, mIndices,
                                            ...
                                            wosx::GeometricQueries<2>& queries)
{
    ...
    wosx::populateGeometricQueriesForDirichletBoundary<2>(absorbingBoundaryHandler, queries);
    ...
    if (!mSolveDoubleSided && mUseSdfForAbsorbingBoundary) {
        ...
        sdfGridForAbsorbingBoundary = std::make_unique<wosx::SdfGrid<2>>(queries.domainMin, queries.domainMax);
        Vector2i sdfGridShape = Vector2i::Constant(mSdfGridResolution);
        wosx::populateSdfGrid<2>(absorbingBoundaryHandler, *sdfGridForAbsorbingBoundary, sdfGridShape);
        wosx::populateGeometricQueriesForDirichletBoundary<wosx::SdfGrid<2>, 2>(*sdfGridForAbsorbingBoundary, queries);
    }
    ...
    std::function<float(float)> branchTraversalWeight = wosx::getBranchTraversalWeightCallback();
    ...
    if (areRobinConditionsPureNeumann) {
        ...
        wosx::populateGeometricQueriesForNeumannBoundary<2>(
        ...
        wosx::populateGeometricQueriesForRobinBoundary<2>(
