...
# This module provides a clean Python API for WoSX by wrapping the C++ nanobind
...
try:
    ...
    BoolList = _wosx.BoolList
    IntList = _wosx.IntList
    UintList = _wosx.UintList
    FloatList = _wosx.FloatList
    convert_list_to_numpy_array = _wosx.convert_list_to_numpy_array

    # Dim-specific list types (keep direct names for convenience)
    Float2List = _wosx.Float2List
    Float3List = _wosx.Float3List
    Int2List = _wosx.Int2List
    Int3List = _wosx.Int3List

    # Dim-dispatched float list constructor
    def FloatNList(*args, dim, **kwargs):
        _validate_dim(dim)
        cls = _wosx.Float2List if dim == 2 else _wosx.Float3List
        return cls(*args, **kwargs)

    # Dim-dispatched int list constructor
    def IntNList(*args, dim, **kwargs):
        _validate_dim(dim)
        cls = _wosx.Int2List if dim == 2 else _wosx.Int3List
        return cls(*args, **kwargs)
    ...
    Core = _wosx.Core
    Solvers = _wosx.Solvers
    Samplers = _wosx.Samplers
    Utils = _wosx.Utils
    OpaqueTypes = _wosx.OpaqueTypes
    ...
    try:
        # Top-level GPU types (non-templated, bound on root module)
        GPUFloat2 = _wosx.GPUFloat2
        GPUFloat3 = _wosx.GPUFloat3

        # Top-level GPU dim+channels factory (bound on root module)
        GPUTaskHandle = _dim_channels_factory(_wosx, "GPUTaskHandle")

        # ==================== GPU Core dispatchers ====================

        Core.GPUKelvinPDE = _dim_channels_factory(Core, "GPUKelvinPDE")

        Core.GPUGeometricQueries = _dim_factory(Core, "GPUGeometricQueries")

        # ==================== GPU Solvers dispatchers ====================

        for _name in [
            "GPUSampleStatistics",
            "GPUSampleStatisticsList",
            "GPUPointEstimator",
            "GPUWalkOnSpheresSolver",
            "GPUWalkOnStarsSolver",
            "GPUBVCEvaluationOutputs",
            "GPUBVCEvaluationOutputsList",
            "GPUBoundaryValueCachingSolver",
        ]:
            setattr(Solvers, _name, _dim_channels_factory(Solvers, _name))
        ...
        for _name in [
            "GPUSamplePoint",
            "GPUSamplePointList",
            "GPUBVCEvaluationPoint",
            "GPUBVCEvaluationPointList",
        ]:
            setattr(Solvers, _name, _dim_factory(Solvers, _name))

        # ==================== GPU Samplers dispatchers ====================

        for _name in [
            "GPUBoundingBoxSolveRegion",
            "GPUWatertightDomainSolveRegion",
            "GPUEmptyDomainSampler",
            "GPUUniformDomainSampler",
            "GPUEmptyBoundarySampler",
            "GPUUniformLineSegmentBoundarySampler",
            "GPUUniformTriangleBoundarySampler",
        ]:
            setattr(Samplers, _name, _dim_factory(Samplers, _name))

        Samplers.create_gpu_uniform_boundary_sampler = _dim_func(
            Samplers, "create_gpu_uniform_boundary_sampler")

        # ==================== GPU Utils dispatchers ====================

        Utils.GPUDenseGrid = _dim_channels_factory(Utils, "GPUDenseGrid")
        Utils.GPUDistanceQueries = _dim_channels_factory(Utils, "GPUDistanceQueries")

        for _name in [
            "GPUSdfGrid",
            "GPUFcpwDirichletBoundaryHandler",
            "GPUSdfDirichletBoundaryHandler",
            "GPUFcpwNeumannBoundaryHandler",
            "GPUFcpwRobinBoundaryHandler",
        ]:
            setattr(Utils, _name, _dim_factory(Utils, _name))

        Utils.get_inside_bounding_box_callback = _dim_func(Utils, "get_inside_bounding_box_callback")
    ...
    import warnings
    warnings.warn(
        f"Failed to import WoSX extension module: {e}\n"
        "This usually means:\n"
